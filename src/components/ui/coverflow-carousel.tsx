...
export interface CoverflowCarouselProps<T = CoverflowSlide> {
  slides: T[];
  renderCard?: (slide: T, index: number, isSelected: boolean) => React.ReactNode;
  ...
  cardWidth?: string;
  /** Optional track/card height. Defaults to var(--cf-card). */
  cardHeight?: string;
...
export function CoverflowCarousel<T = CoverflowSlide>({
  slides,
  renderCard,
  rotate = 44,
  depth = 0.6,
  ...
  const clamp = React.useCallback(
    (pos: number) => (loop && count > 2 ? pos : Math.max(0, Math.min(count - 1, pos))),
    [count, loop],
  );
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    // If clicking on an interactive element inside the card, allow native interaction
  ...
  useIsoLayoutEffect(() => {
    ...
    const measure = () => {
      const card = cardRefs.current.find((c) => c !== null);
      if (!card) return;
  ...
  // Reset/clamp when slides count changes (e.g. filters)
  React.useEffect(() => {
    posRef.current = 0;
    targetRef.current = 0;
    setSelected(0);
  ...
  const active = slides[selected] as CoverflowSlide | undefined;
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                onClick={() => {
                  if (!hasDraggedRef.current && index !== selected) {
                    goTo(index);
                  }
                }}
                className={cn(
              ...
              >
                {renderCard ? (
                  ...
                  <img
                    src={(slide as CoverflowSlide).src}
                    alt={(slide as CoverflowSlide).alt}
                    draggable={false}
        ...
        {showNavigation && count > 1 && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/80 p-2.5 text-foreground backdrop-blur-md transition hover:bg-background hover:scale-110 active:scale-95 border border-border/50 shadow-xl cursor-pointer"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/80 p-2.5 text-foreground backdrop-blur-md transition hover:bg-background hover:scale-110 active:scale-95 border border-border/50 shadow-xl cursor-pointer"
            >
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={cn(
                ...
                index === selected ? "w-6 bg-foreground opacity-100" : "w-2 bg-foreground/30 hover:bg-foreground/60 opacity-30",
              )}
            />
          ))}
        </div>
